...
export default function SettingsModal({ onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70" onClick={onClose}>
      <div
        className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-md p-6 space-y-5"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between">
          <h2 className="font-bold text-zinc-100 text-base">AI Settings</h2>
          <button onClick={onClose} className="text-zinc-500 hover:text-zinc-300 transition-colors text-lg leading-none">✕</button>
        </div>

        {/* Provider */}
        <div>
          <label className="block text-xs font-medium text-zinc-500 mb-2">Provider</label>
            ...
            {(['ollama', 'anthropic'] as const).map((p) => (
              <button
                ...
                className={`flex-1 py-2 text-sm font-medium transition-colors ${
                  settings.provider === p ? 'bg-zinc-700 text-zinc-100' : 'bg-transparent text-zinc-500 hover:text-zinc-300'
                }`}
              >
        ...
        {settings.provider === 'ollama' && (
          <div>
            <label className="block text-xs font-medium text-zinc-500 mb-1.5">Ollama Base URL</label>
            <input
              type="url"
            ...
            <p className="mt-1 text-xs text-zinc-600">
              ...
              <code className="text-zinc-500">OLLAMA_ORIGINS=*</code>
            </p>
          </div>
        )}
        ...
        {settings.provider === 'anthropic' && (
          <div>
            <label className="block text-xs font-medium text-zinc-500 mb-1.5">Anthropic API Key</label>
            <input
              type="password"
              ...
              className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-zinc-200 focus:outline-none focus:border-zinc-500"
            />
            <p className="mt-1 text-xs text-zinc-600">Stored locally on this device only.</p>
          </div>
        )}
